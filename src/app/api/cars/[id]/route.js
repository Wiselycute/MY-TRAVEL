...
export async function PUT(req, { params }) {
  try {
    ...
    const updated = await Car.findByIdAndUpdate(params.id, data, { new: true, runValidators: true }).lean();
    if (!updated) return notFound("Car not found");
...
export async function DELETE(req, { params }) {
  try {
    ...
    const deleted = await Car.findByIdAndDelete(params.id).lean();
    if (!deleted) return notFound("Car not found");
    return NextResponse.json({ success: true, message: "Car deleted" }, { status: 200 });
  } catch (err) {
